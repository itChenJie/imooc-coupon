...
CREATE TABLE IF NOT EXISTS `imooc_coupon_data`.`coupon_template`(
    ...
    `name` varchar(64) NOT NULL DEFAULT '' COMMENT '优惠卷名称',
    `logo` varchar(256) NOT NULL DEFAULT '' COMMENT '优惠卷logo',
    `desdescription` varchar(256) NOT NULL DEFAULT '' COMMENT '优惠卷描述',
    `category` varchar(64) NOT NULL DEFAULT '' COMMENT '优惠卷分类',
    ...
    `template_key` varchar(128) NOT NULL DEFAULT '' COMMENT '优惠卷模版的编码',
    `target` int(11) NOT NULL DEFAULT '0' COMMENT '目标用户',
    `rule` varchar(1024) NOT NULL DEFAULT '' COMMENT '优惠卷规则: TemplateRule 的 json ',
    PRIMARY KEY(`id`),
    KEY `idx_category` (`category`),
    KEY `idx_user_id` (`user_id`),
    UNIQUE KEY `name` (`name`)
)ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=utf8 COMMENT='优惠卷模板表';
